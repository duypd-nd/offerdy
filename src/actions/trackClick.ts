...
export async function trackOfferClick(offerId: string): Promise<void> {
  await Promise.all([
    ...
    writeClient.create({ _type: 'click', offer: { _type: 'reference', _ref: offerId, _weak: true } }),
  ])
}
...
export async function trackStoreClick(storeId: string): Promise<void> {
  await Promise.all([
    ...
    writeClient.create({ _type: 'click', store: { _type: 'reference', _ref: storeId, _weak: true } }),
  ])
}
