...
export default function ComparisonsAdmin({ initialPosts }: { initialPosts: AdminPost[] }) {
  ...
  return (
    <div className="oa-wrap">
      ...
      <div className="oa-header">
        <div>
          <h1 className="oa-title">Quản lý Comparisons</h1>
          <div className="oa-breadcrumb">Home / Comparisons</div>
        </div>
        <a href="/comparisons" target="_blank" rel="noopener noreferrer" className="oa-btn" style={{ display: 'inline-flex', alignItems: 'center', gap: 6, textDecoration: 'none' }}>
      ...
      <div className="oa-table-wrap">
        <table className="oa-table">
          ...
          <tbody>
            ...
            {paginated.length === 0 && <tr><td colSpan={6} className="oa-empty">Chưa có bài so sánh nào</td></tr>}
          </tbody>
        </table>
      </div>
      ...
      {showModal && (
        <ComparisonModal mode="add" onClose={() => setShowModal(false)}
          onSaved={p => { setPosts(prev => [p, ...prev]); setShowModal(false); showToast('Đã thêm bài') }} />
      )}
      {editingPost && (
        <ComparisonModal mode="edit" initial={editingPost} onClose={() => setEditingPost(null)}
...
function ComparisonModal({ mode, initial, onClose, onSaved, onDeleted }: {
  ...
  const handleSlugBlur = async () => {
    ...
    const exists = await checkComparisonSlug(form.slug, mode === 'edit' ? initial?._id : undefined)
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    startTransition(async () => {
      ...
      if (mode === 'add') {
        const doc = await createComparison(data)
        onSaved({ _id: doc._id, _createdAt: new Date().toISOString(), ...data })
      } else if (initial) {
        await updateComparison(initial._id, { ...data, slug: { _type: 'slug', current: form.slug } })
    ...
    <div className="oa-modal-bg">
      <div className="oa-modal oa-modal-lg">
        <div className="oa-modal-head">
          <span>{mode === 'add' ? 'Thêm Comparison' : 'Chỉnh sửa Comparison'}</span>
        ...
        <form className="oa-modal-body" onSubmit={handleSubmit}>
          ...
          <label className="oa-label">Nội dung HTML
            <textarea className="oa-input oa-textarea" rows={8} value={form.content} onChange={e => set('content', e.target.value)}
              placeholder="<p>Nội dung bài so sánh...</p>" style={{ fontFamily: 'monospace', fontSize: 13 }} />
          </label>
          <div className="oa-modal-footer">
            {mode === 'edit' && onDeleted && initial && (
              <button type="button" className="oa-btn oa-btn-red"
                onClick={() => { if (!confirm('Xóa?')) return; startTransition(async () => { await deleteComparison(initial._id); onDeleted(initial._id) }) }}
