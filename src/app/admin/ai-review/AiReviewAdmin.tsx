...
import { approveAiDraft, rejectAiDraft, regenerateAiDraft, approveOfferAiDraft, rejectOfferAiDraft, regenerateOfferAiDraft } from './actions'
...
type OfferAiDraft = { description?: string; generatedAt?: string; model?: string }
...
function OfferReviewPanel({ initialOffers }: { initialOffers: PendingOffer[] }) {
  ...
  const handleApprove = () => {
    if (!selected) return
    startTransition(async () => {
      await approveOfferAiDraft(selected._id, selected.storeSlug, description)
...
export default function AiReviewAdmin({ initialStores, initialOffers }: { initialStores: PendingStore[]; initialOffers: PendingOffer[] }) {
  const [tab, setTab] = useState<'stores' | 'offers'>(initialStores.length > 0 || initialOffers.length === 0 ? 'stores' : 'offers')
